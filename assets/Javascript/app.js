...
database.ref().on("child_added", function (childSnapshot, previousChildKey) {
    ...
    var TimeDiffernce = moment().diff(moment.unix(firebaseTrainTimeInput), "minutes");
    var remainder = moment().deff(moment.unix(firebaseTrainTimeInput), "minutes") % firebaseTrainFrequency;
